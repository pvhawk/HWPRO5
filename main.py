...
def path(path :str):
    def decorate(old_functions):
        def new_functions(*args, **kwargs):
            with open(path, 'a') as f:
                f.write(f'Datetime: {datetime.datetime.now()}; Name: {old_functions.__name__}; Args: {args}; Kwargs: {kwargs}; ')
                results = old_functions(*args, **kwargs)
